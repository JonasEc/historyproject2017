* Manchester Battalion Standardization
* Melanie Wallskog
* 8.1.17

* This do-file standardizes the battalion names in the Manchester battalions.
* It also labels the battalions a la the Long Long Trail website 
*      http://www.longlongtrail.co.uk/army/regiments-and-corps/the-british-infantry-regiments-of-1914-1918/manchester-regiment/

* Key for abbreviations (made by Melanie Wallskog):
...
* Note: I purposely provide an extensive amount of detail below,
* to ease checking code.
...
*******************************************************
* RESERVE BATTALIONS: Brief Label
*******************************************************
...
*******************************************************
* STANDARDIZATION OF FIRST LISTED BATTALION
*******************************************************

gen batt1 = "" /* stores standardized name of first listed battalion */
...
* EXTRANEOUS (some uncertainty, potential typos)
...
replace batt1 = "63 GrB" if (strpos(x,"Grad") > 0 & substr(x,1,2)=="63")
...
*******************************************************
* MISCELANEOUS TYPES, "UNCLEAR" LABEL
*******************************************************

* (coded here, allows for writing over)
replace batt1 = "UC" if (strpos(x,"CB")>0 | strpos(x,"CR")>0 ///
	...
	| strpos(x,"Field Amb")>0 | strpos(x,"Arttilery School")>0 | strpos(x,"3rd Border")>0 ///
	...
	| strpos(x,"Vol Battalion")>0 | strpos(x,"Grand Ba")>0 | x=="3rd Manchester" ///
	...
	| strpos(x,"Naval")>0 | strpos(x,"Deport")>0 | strpos(x,"T R Brig")>0 ///
	| strpos(x,"Counties")>0 | strpos(x,"Chaplain")>0 | strpos(x,"Bery")>0 ///
	| strpos(x,"Imperial")>0 | strpos(x,"Order of the Bath")>0)  
...
replace batt1 = "CoOnly" if x == "3rd Company Battalion" | substr(x,1,21) =="2nd Company Battalion" /* typo */
forval i = 1/30{
...
*******************************************************
* MULTIPLE #'S (not iterations): Use first number
*******************************************************
...
replace batt1 = "23 SB" if substr(x,1,7) == "23rd 3r"
replace batt1 = "2 SB" if substr(x,1,6)== "2nd 6t"
...
replace batt1 = "12 SB" if substr(x,1,6)=="No 12t"
...
replace batt1 = "8 SB" if strpos(x,"8th (a)")>0 | strpos(x,"8th Army B")>0 | strpos(x,"8th (Admiral)") >0 /// 
	| substr(x,1,6)=="8th (A" | substr(x,1,6)=="8th (H" | substr(x,1,6)=="8th A " ///
	| substr(x,1,6)=="8th Te"
replace batt1 = "8 RB" if (substr(x,1,1)=="8" & strpos(x,"National R")>0) | substr(x,1,7)=="8th ( R" ///
	| substr(x,1,6) == "8th (R" | substr(x,1,15) == "8th Natural Res" | substr(x,1,7) == "8th Res" | strpos(x,"National Reserve, 8th")>0
...
replace batt1 = "7 SB" if x == "7th (45th Provisional Battalion)" | strpos(x,"Training Centre, 7th")>0
...
replace batt1 = "4 SB" if x == "Permanent Staff, 4th Battalion"
...
replace batt1 = "3 RB" if strpos(x,"Special Reserve, 3r")>0
replace batt1 = "8 HS" if substr(x,1,6)=="8th( H"
replace batt1 = "24 SB" if substr(x,1,8)=="24th (Ra"
replace batt1 = "19 SB" if substr(x,1,7)=="19th (E"
replace batt1 = "506 HS" if substr(x,1,10)=="506th Home"
replace batt1 = "394 Bty" if x == "394th Siege Battary"
replace batt1 = "34 SB" if strpos(x,"Training Centre, 34th")>0
replace batt1 = "136 SB" if substr(x,1,8)=="136th (S"
replace batt1 = "10 RB" if substr(x,1,8) == "10th ( R"
replace batt1 = "14 RB" if strpos(x,"Training Reserve, 14th")>0
replace batt1 = "9 SB" if substr(x,1,6)=="9th (M"
replace batt1 = "8.1 SB" if substr(x,1,10) == "1st (A) 8t" /* I label this as 8.1, since I've only seen (A) documentation for 8th bat*/

*******************************************************
* TYPE OF BATTALIONS
*******************************************************

* determined via the listings on the Long Long Trail
gen type1 = ""
...
replace type1 = "Territorial Force" if ( ///
	...
	| batt1 == "28 SB" | strpos(x,"Vol Battalion")>0)
...
replace type1 = "Misc" if ( ///
	...
	| strpos(batt1, "En")>0)
replace type1 = "Unknown" if type1==""

*******************************************************
* CHECKS
*******************************************************
save data/ManBattStandardize.dta, replace

/* Reserve
gen reserve_post = (strpos(batt1,"RB")>0)
gen diff_reserve = reserve_post - reserve
count if diff_reserve !=0 /* 1,204: Highlights problem of multiple battalions listed, abbreviations */

* vs. Jonas
gen reg_mw = (type1== "Regular")
gen tf_mw = (type1=="Territorial Force")
gen na_mw = (type1 =="New Armies")
gen pal_mw = (type1 == "Pals")
gen oth_mw = (type1 == "Other")
gen misc_mw = (type1 == "Misc")
gen unk_mw = (type1 == "Unknown")

gen reg_jmg = (type_jmg=="Regular")
gen tf_jmg = (type_jmg =="TF")
gen na_jmg = (type_jmg =="New Armies")
gen pal_jmg = (type_jmg == "Pal")
gen oth_jmg = (type_jmg == "Other")
gen unk_jmg = (type_jmg == "Unclear")

gen diff_reg = reg_mw - reg_jmg
gen diff_tf = tf_mw - tf_jmg
gen diff_na = na_mw - na_jmg
gen diff_pal = pal_mw - pal_jmg
gen diff_oth = oth_mw - oth_jmg
gen diff_unk = unk_mw - unk_jmg

/* Prevalence of types
encode type1, gen(etype1)
preserve
collapse (sum) howmany, by(etype1)
* Total: 59,424
* Misc: 3648 (6.14%)
* New Armies: 4874 (8.2%)
* Other: 1414 (2.38%)
