...
**** 1939 occupations
keep id occupation_x
collapse (count) id, by(occupation_x)
rename id N
lab var N "frequency of occupation"
count /* 2,733 unique occupations */
export delimited "occup1939.csv", replace

* save thousands as each csv
keep if _n<=1000
export delimited "occup1939_b1.csv", replace
clear
import delimited "occup1939.csv"
keep if _n>1000 & _n<=2000
export delimited "occup1939_b2.csv", replace
clear
import delimited "occup1939.csv"
keep if _n>2000 & _n<=3000
export delimited "occup1939_b3.csv", replace

clear


**** 1914/1915 occupations
import delimited "SC1939FirstSampleJustOccup.csv"
keep id occupation_y
collapse (count) id, by(occupation_y)
rename id N
lab var N "frequency of occupation"
count /* 612 unique occupations */
export delimited "occup1914.csv", replace

clear


************************************
* Run data through http://historyofwork.iisg.nl/coding.php
	* Note, only accepts 1000 at a time, so must doe 1939 in 3 batches
	
	
* 1914/1915
* Initial push: 228 matched, 384 unmatched

* 1939
* first 1000 (b1)
* Initial push: 46 matched, 954 unmatched

* second 1000 (b2)
* Initial push: 47 matched, 953 unmatched

* last batch (b3)
* Initial push: 37 matched, 696 unmatched

* 1939 total initial push: 130 matched, 2603 unmatched


**************************************
* Clean datasets to get more matches

* 1914
clear
import delimited "hisco1914_v1.csv"
...
gen matched = (hisco_occ != "")
save "hisco1914_v1.dta", replace
* 1939 b1
clear
import delimited "hisco1939_b1_v1.csv"
drop if _n==1 /* headers didn't import */
keep v1 v3 v7 
...
gen matched = (hisco_occ != "")
save "hisco1939_b1_v1.dta", replace

* 1939 b2

clear
import delimited "hisco1939_b2_v1.csv"
drop if _n==1 /* headers didn't import */
keep v1 v3 v7 
...
gen matched = (hisco_occ != "")
save "hisco1939_b2_v1.dta", replace

* 1939 b3

clear
import delimited "hisco1939_b3_v1.csv"
drop if _n==1 /* headers didn't import */
keep v1 v3 v7 
...
gen matched = (hisco_occ != "")
save "hisco1939_b3_v1.dta", replace


* Merge datasets for consistency
append using "hisco1939_b2_v1.dta"
append using "hisco1939_b1_v1.dta"
append using "hisco1914_v1.dta"

save "hiscoALL_v1.dta", replacoe

gen sort_des = -is1914 /* sort descending order */
sort sort_des matched occ
order occ matched
...
`w' = "Manufacturing" if strpos(occ,"Munitions")>0 & occ_rev=="" /* couln't find anything better*/
...
replace revised = (occ_rev!="")
sort sort_des revised occ
order occ revised

replace occ_rev = lower(occ_rev)

save "hiscoALL_v2.dta", replace

* Export 1914 for analysis
keep if is1914==1

save "hisco1914_v2.dta", replace

export delimited "occup1914_v2.csv", replace


* Run through HISCO website
* This is currently annoying because in cases where there are similar occupations
* in HISCO, the site does not automatically match but forces you to manually accept the simplest match.

clear
import delimited "hisco1914_v2.csv"
drop if _n==1 /* headers didn't import */
keep v1 v3 v7 
...
replace hisco_occ = "Nil/Unclear" if occ=="nil/unclear"
replace hisco = -1 if hisco_occ=="Nil/Unclear"

rename occ occ_rev
*rename hisco_occ occ_rev

collapse (mean) hisclass, by(occ_rev)

save "hisclass1914_v1.dta", replace 

* merge back in
merge 1:m occ_rev using "hisco1914_v2.dta"

drop sort_des _merge hisco_occ hisco_code revised
lab var matched "=1 if matched initially without revision"

rename occ occupation_y
save "hisclass1914_v2.dta", replace

clear
* merge into full sample (just 1914)
import delimited "SC1939FirstSampleJustOccup.csv"

merge m:1 occupation_y using "hisclass1914_v2.dta"
...
replace revised=1 if occ_rev!=""

sort is1914 revised occ

replace occ_rev = lower(occ_rev)

save "hiscoALL_v3.dta", replace

* Export 1939 for analysis
keep if is1914==0

save "hisco1939_v2.dta", replace

* first, collapse to speed up matching process
collapse (first) occ, by(occ_rev)

export delimited "occup1939_v2.csv", replace

* Run through HISCO website

clear
import delimited "hisco1939_v2.csv"
drop if _n==1 /* headers didn't import */
keep v1 v3 v7 
...
save "hisclass1939_v1.dta", replace 

* merge back in
merge 1:m occ_rev using "hisco1939_v2.dta"

drop sort_des _merge hisco_occ hisco_code revised
lab var matched "=1 if matched initially without revision"

rename occ occupation_x

* somehow we have a few repeats

collapse (first) hisco occ_rev hisclass matched is1914, by(occupation_x)

save "hisclass1939_v2.dta", replace

clear
* merge into full sample, with 1914 already done
import delimited "FirstSample_1914hisclass.csv"

merge m:1 occupation_x using "hisclass1939_v2.dta", gen(_merge2)
