...
def scrape(url,maxPageName):
	...
	for page in range(maxPageName):
		urlUse = url + "&page=" + str(page)
		while True:
			try:
				driver.open(urlUse) #if fail, wait 90 sec (on average)
				break
			except:
...
baseurl = "https://search.livesofthefirstworldwar.org/search/world-records/british-army-service-records?regiment=manchester%20regiment&birthtown="
...
if choice == 1:
	...
	# open each webpage individually
		...
		hitnumbers = hitnumbers + [hits]

		if hits == 0:
			continue
		elif hits >= entries*maxPage:
			dummy = 2
		else:
			dummy = 0

		maxPageName = min(int(ceil(hits/entries)),maxPage)

		if dummy == 0:
			output = output + scrape(url,maxPageName)
		if dummy == 2:
			for name in (nameList1 + nameList2):
				url = baseurl + birthtown + nameSelector + name
	...
	## find out what we are dealing with:
		...
		hits = hitsHeader.text
		hits = hits.replace(',',"")
		hits = [int(s) for s in hits.split() if s.isdigit()][0]
